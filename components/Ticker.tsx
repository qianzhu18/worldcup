...
type Item = { label: string; value: string; code?: string; up?: boolean };

export function Ticker({ items }: { items: Item[] }) {
  if (!items.length) return null;
  const loop = [...items, ...items]; // seamless 50% marquee
  return (
    <div className="relative border-y border-emerald-400/15 bg-[#07121b]/82 backdrop-blur">
      <div className="absolute left-0 top-0 z-10 flex h-full items-center gap-1.5 bg-emerald-400 px-3 text-[11px] font-bold uppercase tracking-wider text-[#04130c] shadow-glow-electric">
        <span className="h-1.5 w-1.5 animate-pulse rounded-full bg-[#04130c]" />
        LIVE
      </div>
      <div className="marquee-mask overflow-hidden py-2 pl-20">
        <div className="flex w-max animate-marquee gap-8 will-change-transform hover:[animation-play-state:paused]">
          {loop.map((it, i) => (
            <span key={i} className="flex items-center gap-2 whitespace-nowrap text-sm">
              ...
              <span className="font-bold tabular-nums text-emerald-300">{it.value}</span>
