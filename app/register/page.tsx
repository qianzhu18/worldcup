...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (signUpError) {
      setError(signUpError.message.includes("already") ? "该邮箱已注册" : "注册失败，请稍后重试");
      return;
    }
      ...
      <div className="w-full max-w-md">
        <div className="card p-8">
          <div className="mb-8 text-center">
            ...
            <p className="mt-2 text-sm text-slate-400">创建账号，开始你的世界杯预测之旅</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {message && <div className="rounded-lg bg-emerald-500/10 px-4 py-2 text-sm text-emerald-300">{message}</div>}
