...
这个项目如果用于产品实习求职，重点不是“功能完整度最高”，而是要证明：

> 我不是只会做 demo，而是能发现问题、提出假设、上线验证、观察数据、快速优化。

这和产品实习的核心要求一致：**思路、链路、数据，比功能数量更重要。**
...
```text
背景是什么 -> 发现什么问题 -> 为什么判断是痛点 -> 做了什么实验 -> 如何优化 -> 数据变化 -> 结论是什么
```

套到 JMWL 上就是：

1. 背景：世界杯期间用户有强烈看球需求，但普通赛程表只告诉用户“几点踢”，不能告诉用户“哪场值得看、怎么看”。
2. 问题：原产品像预测看板，用户看完胜率就走，缺少长期留存和反馈。
3. 假设：如果把胜率看板改成“赛前策略天气 + 模拟采纳 + 赛后复盘”，用户会更愿意反复使用。
4. 实验：上线策略页和策略卡，邀请小规模用户使用。
5. 优化：根据点击和反馈调整入口、文案、策略类型和复盘方式。
6. 数据：观察策略卡点击率、采纳率、赛后回访率、分享率。
7. 结论：验证用户是否只想看比分，还是愿意使用 AI 策略陪伴。
...
| 指标 | 说明 | 为什么重要 |
| --- | --- | --- |
| 访问人数 | 有多少人看过策略页 | 证明推广触达 |
| 策略卡浏览率 | 进入策略页后是否继续看具体比赛 | 证明内容吸引力 |
| 采纳策略率 | 用户是否点击“采纳策略” | 证明策略有行动价值 |
| 观望点击率 | 用户是否接受“这场不推荐” | 证明 AI 劝退有价值 |
| 赛后复盘查看率 | 用户是否回来查看结果 | 证明闭环成立 |
| 分享卡生成率 | 用户是否愿意把结果发出去 | 证明传播潜力 |
| 文字反馈数 | 用户真实说了什么 | 证明不是闭门造车 |

### 13.3 数据埋点建议

MVP 阶段可以不用上复杂 BI，先做轻量事件记录。
...
strategy_page_view
match_strategy_view
strategy_adopt_click
strategy_watch_click
strategy_reverse_click
strategy_share_click
review_page_view
profile_page_view
```
...
- `match_id`。
- `strategy_type`。
- `source`：微信群、小红书、朋友圈、直接访问等。
- `created_at`。

这样后续就能讲清楚：

> 我不是凭感觉判断，而是通过事件数据发现用户更愿意点击“稳健策略”，但对“杠铃策略”理解成本高，所以第二版优化了策略解释文案。
...
> 独立设计并上线 AI 看球策略助手 MVP，面向世界杯看球场景设计“赛前策略天气 - 模拟采纳 - 赛后复盘”闭环；通过 7 天小规模运营实验，跟踪策略页访问、策略卡点击、采纳率、复盘回访和用户反馈，验证用户对赛前决策辅助的需求。

如果拿到更具体数据，可以写：

> 7 天内触达 50+ 用户，获得 20+ 次策略采纳，采纳率约 X%，赛后复盘回访率约 Y%；基于用户反馈优化策略卡文案和入口层级，提升策略卡点击率 Z%。

注意：数字不用夸大。真实、小样本、有因果链条，比虚假的大数字更像产品经理。
...
> 今晚世界杯 3 场，JMWL 只推荐看这 1 场。  
> 市场看好德国，但模型认为法国方向有 +5pt Edge。  
> 我把完整策略卡放这里。
...
这个项目的求职价值不在于“我做了一个世界杯网站”。

真正应该表达的是：

> 我把一个一次性胜率查询工具，重构成了一个可运营、可追踪、可复盘的 AI 看球策略产品，并用小规模用户数据验证了产品假设。

这句话比“我做了一个 Next.js + Supabase 项目”更适合产品岗。
